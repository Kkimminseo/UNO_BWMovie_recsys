...
class MoviePreference(models.Model):
    user_id_fk = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="genre_preference"
    )  # FK "유저 ID"
    movie_id_fk = models.ForeignKey(
        Movie, on_delete=models.CASCADE, related_name="movie_preference"
    )  # FK "영화 ID"
...
class GenrePreference(models.Model):
    ...
    def __str__(self):
        return f"{self.user_id_fk} liked/disliked {self.genre_id}."
